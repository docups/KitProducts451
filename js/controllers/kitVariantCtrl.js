...
function ($scope, $location, $route, $routeParams, $window, ProductDisplayService, Variant, Order) {
    ...
    function init() {
        ...
        ProductDisplayService.getProductAndVariant($routeParams.productInteropID, varID, function(data){
            ...
            var kitSpecs = store.get('451Cache.KitSpecs') ? store.get('451Cache.KitSpecs') : {};
            if(varID)
                ...
                if($scope.Variant.Specs){
                    angular.forEach($scope.Variant.Specs, function(spec){
                        angular.forEach(kitSpecs, function(kitspec){
                            if(spec.Name === kitspec.Name){
                                spec.Value = kitspec.Value;
                            }
                        });
                    });
                }
